    ...
    return {
        ...
        rules: {
            ...
            notIncludeField: {
                ...
                validate: (value: string, helpers: joi.CustomHelpers, { fields = [] }: { fields: string[] }) => {
                    ...
                    if (objectValue) {
                        const isIncludes = fields.filter(
                            (item) =>
                                ...
                                objectValue[item].split(' ').some((str) => value.includes(str)),
                        );

                        if (Boolean(isIncludes.length)) {
                            return helpers.error('password.notIncludeField', { field: isIncludes[0] });
                        }
                    }
                    return value;
                },
